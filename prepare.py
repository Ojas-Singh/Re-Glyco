...
pcaG.to_csv(config.data_dir+name+"/"+name+'_G_pca.csv',index_label="i")
...
pcaT.to_csv(config.data_dir+name+"/"+name+'_T_pca.csv',index_label="i")  

tsneT = clustering.tsnewithT(tor,config.number_of_dimensions)
tsneT.to_csv(config.data_dir+name+"/"+name+'_T_tsne.csv',index_label="i") 
